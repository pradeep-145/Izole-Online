    ...
    (set, get) => ({
      ...
      login: async (email, password) => {
        ...
        try {
          const response = await axios.post(
            `${import.meta.env.VITE_SERVER_URL}https://uzlmegb12i.execute-api.ap-south-1.amazonaws.com/api/admin/login`,
            {
              email,
              password,
            }
        ...
        pendingRequests.products[requestKey] = new Promise(async (resolve) => {
          try {
            ...
            const response = await axios.get(
              `${import.meta.env.VITE_SERVER_URL}https://uzlmegb12i.execute-api.ap-south-1.amazonaws.com/api/products?${queryParams}`,
              {
                headers: {
      ...
      fetchDashboardAnalytics: async (forceRefresh = false) => {
        ...
        pendingRequests.analytics = new Promise(async (resolve) => {
          try {
            const response = await axios.get(
              `${import.meta.env.VITE_SERVER_URL}https://uzlmegb12i.execute-api.ap-south-1.amazonaws.com/api/admin/analytics`,
              {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
                },
